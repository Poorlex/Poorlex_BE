...
public interface ExpenditureRepository extends JpaRepository<Expenditure, Long> {
    ...
    Long findTotalExpenditureByMemberIdBetweenDate(@Param(value = "memberId") final Long memberId,
                                                   @Param(value = "start") final LocalDate start,
                                                   @Param(value = "end") final LocalDate end);

    @Query(value = "select coalesce(sum(e.amount.value), 0) from Expenditure e "
        + "where e.memberId = :memberId and e.date between :start and :end")
    Long findTotalExpenditureByMemberIdBetweenDateTime(@Param(value = "memberId") final Long memberId,
    ...
    List<Expenditure> findExpenditureByMemberId(final Long memberId, final Pageable pageable);
