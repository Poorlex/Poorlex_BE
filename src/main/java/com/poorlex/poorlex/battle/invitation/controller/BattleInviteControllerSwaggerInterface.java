...
public interface BattleInviteControllerSwaggerInterface {
    ...
    ResponseEntity<Void> invite(@Parameter(description = "초대할 배틀 ID") final Long battleId,
    ...
    ResponseEntity<Void> inviteAccept(@Parameter(hidden = true) final MemberInfo memberInfo,
                                      @Parameter(description = "초대를 보낸 배틀참가자 ID") final BattleInviteAcceptRequest request);

    @Operation(summary = "배틀 초대 거절", description = "액세스 토큰 필요")
    @ApiResponse(responseCode = "200")
    @PostMapping("/battle-invite/deny")
    ResponseEntity<Void> inviteAccept(@Parameter(hidden = true) final MemberInfo memberInfo,
                                      @Parameter(description = "초대를 보낸 배틀참가자 ID") final BattleInviteDenyRequest request);
}
