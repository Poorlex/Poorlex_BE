...
public interface ExpenditureRepository extends JpaRepository<Expenditure, Long> {
    ...
    @Query(value = "select coalesce(sum(e.amount.value), 0) from Expenditure e "
        + "where e.memberId = :memberId and e.dateTime between :start and :end")
    ...
    @Query(value = "select m.id as memberId, coalesce(sum(e.amount.value), 0) as totalExpenditure "
        + "from Member m left join Expenditure e on m.id = e.memberId and e.dateTime between :start and :end "
    ...
    @Query("select e from Expenditure e "
        ...
        + "where e.dateTime between b.duration.start and b.duration.end")
    List<Expenditure> findBattleExpenditureByBattleId(final Long battleId);

    List<Expenditure> findExpendituresByMemberIdAndDateTimeBetween(final Long memberId,
